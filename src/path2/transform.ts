...
export function pathArrToB3Path(pathArr: (string | number)[][]): B3Path {
    ...
    for (let i = 1; i < pathArr.length; i++) {
        ...
        const p0 = Point.make(seg[0], seg[1]);
        const p1 = Point.make(seg[2], seg[3]);
        const p2 = Point.make(seg[4], seg[5]);
        const p3 = Point.make(seg[6], seg[7]);
        if (p0.equals(p1) && p3.equals(p2)) {
            path.push(Line.make(p0, p3));
        }
        else {
            path.push(B3Curve.make(p0, p1, p2, p3));
        }
    }
    return path;
}

export function b3pathToArr(path: B3Path): (string | number)[][] {
