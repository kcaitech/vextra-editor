...
const extend = ref<boolean>(props.context.attr.textMaskFold);

const textCtxMgr = new TextContextMgr(props.context, {} as TextContext);

const changeFold = () => {
    extend.value = !extend.value;
    props.context.attr.setTextMaskFold();
}
</script>
...
<template>
    <div class="container">
        <div class="header" @click="changeFold">
            <span>{{ t('stylelib.format') }}</span>
            <div class="down">
                <SvgIcon :icon="down_icon" :style="{ transform: extend ? 'rotate(0deg)' : 'rotate(-90deg)' }" />
            </div>
        </div>
        <template v-if="extend" v-for="c in data" :key="c.id">
            <TextMaskPanelItem v-if="!c.disabled" :context="context" :data="c" :manager="textCtxMgr"></TextMaskPanelItem>
        </template>
    </div>
</template>
