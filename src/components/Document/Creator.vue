    ...
    if (newShape) {
        if (newShape.type === ShapeType.Text) {
            ...
            if (page && asyncCreator) {
                asyncCreator.collect(page.data, children.map((s) => adapt2Shape(s)), adapt2Shape(props.context.selection.selectedShapes[0]) as Artboard);
            }
        }
        removeCreator();
