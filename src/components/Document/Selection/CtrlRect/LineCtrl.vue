...
const clipPath = computed<string>(() => {
    ...
    return `path('M 0, 0 l ${width}, ${height} l -10, 0 l (10 - ${width}), (10 - ${height}) z')`
});
const points = computed<CPoint[]>(() => {
    const { width, height } = props.controllerFrame;
    const p1: CPoint = [CtrlElementType.RectLT, 0 - borderWidth, 0 - borderWidth, 'move']
    const p2: CPoint = [CtrlElementType.RectRB, width - 2 * borderWidth, height - 2 * borderWidth, 'move']
    const ps: CPoint[] = [p1, p2];
    ps.forEach(p => {
        p[1] -= offset;
        p[2] -= offset;
    })
...
<template>
    <div class="ctrl-rect" @mousedown="mousedown" :style="{
        ...
        <div class="frame" :style="{
            ...
            <span>{{ `${props.controllerFrame.realWidth.toFixed(2)} * 0` }}</span>
        </div>
    </div>
</template>
