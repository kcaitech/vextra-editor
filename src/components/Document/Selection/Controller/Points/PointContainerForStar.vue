...
const innerAngleDotEl = ref<SVGGElement>();
const radiusDotEl = ref<SVGGElement>();
let changeType: PointActionType | undefined;
...
const update_dot_position = () => {
    ...
    const inner_angle = getInnerAnglePosition();
    const radius = getRadiusPosition();
    if (inner_angle) inner_angle_dot.value = inner_angle;
...
function getCountPosition() {
    ...
    const cornerInfo = getCornerControlPoint(shape.points, 2, props.shape.frame);
...
function getInnerAnglePosition() {
    const shape = props.shape.data;
    inner_angle.value = shape.innerAngle * 100;
...
<template>
    ...
    <!-- 内角 -->
    <g :style="`transform: translate(${inner_angle_dot.x - 4}px, ${inner_angle_dot.y - 4}px);`" ref="innerAngleDotEl"
        @mousedown.stop="(e) => point_mousedown(e, PointActionType.InnerAngle)" @mousemove="dot_mousemove"
        @mouseenter="(e) => point_mouseenter(e, PointActionType.InnerAngle)" @mouseleave="point_mouseleave">
        <ellipse cx="4" cy="4" rx="5" ry="5" fill="transparent" fill-opacity="1" />
        <ellipse cx="4" cy="4" rx="4" ry="4" fill="#FFFFFF" fill-opacity="1" />
        <ellipse cx="4" cy="4" rx="4" ry="4" fill-opacity="0" stroke-opacity="1" stroke="#1878F5" fill="none"
            stroke-width="1" />
