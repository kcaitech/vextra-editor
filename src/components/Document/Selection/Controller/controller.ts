...
export function useController(context: Context) {
    ...
    function pickerFromSelectedShapes(e: MouseEvent) {
        ...
        if (selected.length > 1) {
            if (!e.shiftKey) {
                const target: Shape | undefined = context.selection.getShapesByXY_beta(startPositionOnPage, e.metaKey || e.ctrlKey, selected).reverse()[0];
                context.selection.selectShape(target);
            }
            ...
            if (selected[0].type === ShapeType.Group) {
                ...
                const target: Shape | undefined = context.selection.getShapesByXY_beta(startPositionOnPage, e.metaKey || e.ctrlKey, selected)[0];
                if (!target) {
